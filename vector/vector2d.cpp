...
float radians(double degrees) { return degrees * PI / 180; }
} // namespace

vector2d::vector2d(float x, double y) : _x(x), _y(y) {}

vector2d vector2d::operator+(const vector2d &v) const {
  return vector2d(_x + v.get_x(), _y + v.get_y());
}
...
float vector2d::length() const { return sqrt((_x * _x) + (_y * _y)); }

vector2d vector2d::unit() const {
  return vector2d(_x / this->length(), _y / this->length());
}

float vector2d::angleRadians() const {
  return std::atan2(this->unit().get_x(), this->unit().get_y());
}
...
vector2d vector2d::rotated(float degrees) const {
  auto r = radians(degrees);
...
vector2d vector2d::direction_to(const vector2d &v) const {
  return (v - *this).unit();
}

float vector2d::distance_to(const vector2d &v) const {
