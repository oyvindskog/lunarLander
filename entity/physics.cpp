...
physics::thrust()
{
    impact(get_direction_vector() * _force);
}

void
physics::accelerate_angular()
{
    _angular_speed += _angular_acceleration; 
}
...
void
physics::impact(const vector2d& force)
{
    _acceleration += force;
}

void
physics::impact_angular(float force)
{
    _angular_acceleration += force;
}
