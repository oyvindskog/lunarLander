...
collision::collision(entity *ent) : component(ent) {
...
void collision::handle_collisions(entity_vector &entities) {
  for (auto &e : entities) {
    if (e.get() == _ent || !e->has_component<collision>())
      continue;
    ...
    for (auto &p : e_body->get_parts()) {
      for (auto &this_p : _body->get_parts()) {
        ...
        if (bodyparts_overlap(pp, this_pp)) {
          ...
          vector2d pNonRotated =
              this_p.v.rotated(270 - _physics->get_direction());
